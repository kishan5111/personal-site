...
```
Compute ceiling:   Peak tensor throughput (e.g. ~989 dense BF16 tensor TFLOPS on H100 SXM)
...
Using a simplified dense-BF16 roofline approximation, the H100 SXM has a ridge point (the batch size where this GEMM view crosses from memory-bound to compute-bound) of roughly. NVIDIA publishes **1,979 BF16 Tensor TFLOPS with sparsity** for H100 SXM, which implies about **989 dense BF16 Tensor TFLOPS** once you back out the sparsity uplift:
...
Meaning: under this simplified model, you need an effective batch around `295` before this approximation stops looking clearly bandwidth-dominated. Real decode kernels, cache reuse, quantization, scheduler behavior, and the fact that real serving traces do not look like ideal GEMMs will move that threshold, but the important point survives: at batch `= 1`, decode sits deep in the memory-bound regime.
...
INT4 quantization: `AI ≈ 4 × batch`. Under the same H100 SXM approximation, the ridge point drops from about `295` to about `74`. That is still high enough that small-batch decode remains memory-bound, but much lower than the BF16 case.
...
**Agentic systems make this significantly worse.** A tool-calling agent typically passes the full conversation history, including tool outputs — back as context on every call. An agent running 10 steps with 500-token tool responses per step accumulates 5,000+ tokens of context before it finishes. That's a 5K-token prefill on every subsequent call, each building a fresh KV cache. On a multi-agent setup with parallel workers, this multiplies across however many concurrent agent threads you are running.
...
Streaming multiprocessors (SMs) are where the math happens. To understand GPU architecture more deeply, I recommend watching [How do Graphics Cards Work? Exploring GPU Architecture](https://www.youtube.com/watch?v=h9Z4oGN89MU) by Branch Education.
...
**They are nearly tied at small batch, and H100 pulls ahead as batch grows.**
...
| Metric                                               | RTX Pro 6000 BW | H100 SXM        | Ratio (H100/RTX)   | Expected (from bandwidth ratio) |
| ---------------------------------------------------- | --------------- | --------------- | ------------------ | -------- |
...
**What the theory predicted vs what I found:**

* At `bs=1`, decode stayed much closer than raw bandwidth ratio alone would suggest.
* As batch and prompt length grew, H100's advantage became more visible.
* RTX's extra VRAM still matters in fit-constrained regimes even when H100 wins on throughput.
* The broad direction held, but the measured gap was smaller and more workload-dependent than the spec sheet implies.
...
* At small batch sizes, only a few experts fire per step — low compute utilization, irregular memory access patterns
...
* **P-EAGLE** — Parallel-EAGLE3 drafting. P-EAGLE runs multiple draft heads in parallel to improve acceptance rates at larger batch sizes.
...
If the model barely fits, maximize **VRAM first**. Once you start offloading weights or KV cache, you are not in an optimization problem anymore. You are in a survival problem.

If the workload is decode-heavy, maximize **memory bandwidth**. Typical examples:
...
If the workload is prefill-heavy, maximize **compute throughput** and kernel quality. Typical examples:
...
If multi-GPU inference is unavoidable, favor **NVLink over PCIe** whenever you can.
...
From there, the practical implications are straightforward. Decode is usually memory bound at real serving batch sizes, while prefill is much more compute-friendly. KV cache, not just weights, determines whether the deployment remains healthy under concurrency. If you do not know your prefill/decode mix, you are guessing.

That is why H100 pulling ahead on higher-batch decode is not mysterious. It is the expected result once you frame the workload correctly.
