...
const extractMarkdownTableOfContents = (content?: string): TocItem[] => {
  if (!content) {
    return [];
  }
...
const BlogPost = () => {
  ...
  const tocItems: TocItem[] = post?.sections?.length
    ? post.sections.map((section) => ({
        id: section.id,
        title: section.title,
        level: 2 as const,
      }))
    : extractMarkdownTableOfContents(post?.content);
  ...
  useEffect(() => {
    if (!post?.content || post.sections || !articleRef.current || !tocItems.length) {
      return;
    }
